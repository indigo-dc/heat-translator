...
class HotTemplate(object):
    ...
    def output_to_yaml_files_dict(self, base_filename,
        ...
        substacks_resources_to_remove = []
        for resource in self.resources:
            substacks, substack_resources = resource.\
                extract_substack_templates(self.resources, base_filename,
                                           hot_template_version)
            yaml_files_dict.update(substacks)
            substacks_resources_to_remove.extend(substack_resources)

        for r in substacks_resources_to_remove:
    ...
    def output_to_yaml(self, embed_substack_templates=True,
        ...
        if embed_substack_templates:
            ...
            substacks_resources_to_remove = []
            for resource in self.resources:
                substacks_resources_to_remove.extend(
                    resource.embed_substack_templates(self.resources,
                                                      hot_template_version))
            for r in substacks_resources_to_remove:
                self.resources.remove(r)
